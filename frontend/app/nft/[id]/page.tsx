...
import { Heart, Share2, ExternalLink, Volume2, AlertCircle } from "lucide-react"
...
export default function NFTDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
          ...
          <div className="grid lg:grid-cols-2 gap-12">
              ...
              {nft.audio && (
                <Card className="bg-slate-900/50 backdrop-blur-sm border-slate-700/50">
                  <CardContent className="p-6">
                    <h3 className="text-lg font-semibold mb-4 text-purple-300">Audio Player</h3>
              ...
              {nft.type === "audio" && !nft.audio && (
                <Card className="bg-slate-900/50 backdrop-blur-sm border-slate-700/50">
                  <CardContent className="p-6 text-center">
                    <h3 className="text-lg font-semibold mb-4 text-purple-300">Audio Preview</h3>
                    <div className="bg-slate-800/50 rounded-xl p-6 backdrop-blur-sm">
                      <AlertCircle className="w-12 h-12 mx-auto mb-4 text-amber-400" />
                      <p className="text-slate-300">Audio preview is not available for this NFT.</p>
                    </div>
                  </CardContent>
                </Card>
              )}
            ...
            >
              ...
              <div>
                ...
                <p className="text-slate-300 text-lg leading-relaxed">{nft.description}</p>
              </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      ...
                      <p className="text-sm text-slate-400">≈ ${(nft.price * 2500).toLocaleString()}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-slate-400 mb-1">Token ID</p>
                      <p className="font-mono text-purple-300">#{nft.tokenId}</p>
                    </div>
                  </div>
              ...
              <Card className="bg-slate-900/50 backdrop-blur-sm border-slate-700/50">
                <CardContent className="p-6">
                  <h3 className="text-lg font-semibold mb-4 text-purple-300">Royalty Information</h3>
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-slate-400">Creator Earnings per Sale</span>
                      <span className="text-purple-300 font-medium">{(nft.price * 0.9).toFixed(3)} ETH</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              {nft.tags && nft.tags.length > 0 && (
                <Card className="bg-slate-900/50 backdrop-blur-sm border-slate-700/50">
                  <CardContent className="p-6">
                    <h3 className="text-lg font-semibold mb-4 text-purple-300">Tags</h3>
              ...
              <Card className="bg-slate-900/50 backdrop-blur-sm border-slate-700/50">
                <CardContent className="p-6">
                  <h3 className="text-lg font-semibold mb-4 text-purple-300">Contract Details</h3>
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between items-center">
                      <span className="text-slate-400">Contract Address</span>
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-slate-300">0x1234...5678</span>
      ...
      <PurchaseModal nft={nft} isOpen={showPurchaseModal} onClose={() => setShowPurchaseModal(false)} />
    </div>
  )
}
