        ...
        if (playPromise !== undefined) {
          playPromise
            ...
            .catch((error) => {
              ...
              setHasError(true)
              toast.error("Unable to play audio: " + (error.message || "Unknown error"))
            })
        }
      } catch (error) {
  ...
  if (hasError) {
    return (
      <div className="bg-slate-800/50 rounded-xl p-6 backdrop-blur-sm text-center">
        ...
        <h3 className="text-lg font-semibold text-white mb-2">Audio Unavailable</h3>
        <p className="text-slate-400">
          This audio content is currently unavailable. The creator may need to upload the file.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-slate-800/50 rounded-xl p-6 backdrop-blur-sm">
        ...
        <div className="h-20 bg-slate-700/50 rounded-lg flex items-end justify-center gap-1 p-2">
          {Array.from({ length: 50 }).map((_, i) => (
            <motion.div
              ...
              animate={{
                height: isPlaying ? `${Math.random() * 60 + 10}%` : `${Math.random() * 60 + 10}%`,
              }}
              transition={{
                duration: 0.5,
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={toggleMute} className="p-2">
