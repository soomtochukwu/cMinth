...
export function FileUpload({ onFilesUploaded }: FileUploadProps) {
  ...
  const simulateUpload = async (file: File, type: "main" | "artwork") => {
    setIsUploading(true)
    ...
    toast.success(`${type === "main" ? "Main file" : "Artwork"} uploaded successfully!`)
  }
  ...
  const removeFile = (type: "main" | "artwork") => {
    setUploadedFiles((prev) => {
      const newFiles = { ...prev }
  ...
  return (
    <div className="space-y-6">
      {/* Main File Upload */}
      <div>
        <h3 className="text-lg font-semibold mb-3 text-white">Main Content *</h3>
            ...
            <CardContent {...getMainRootProps()} className="p-8">
              ...
              <AnimatePresence mode="wait">
                {uploadedFiles.main ? (
                  ...
                  >
                    <div className="flex items-center justify-center mb-4">{getFileIcon(uploadedFiles.main)}</div>
                    <h4 className="text-lg font-semibold text-white mb-2">{uploadedFiles.main.name}</h4>
                    <p className="text-slate-400 mb-4">{(uploadedFiles.main.size / 1024 / 1024).toFixed(2)} MB</p>
                  ...
                  >
                    ...
                    <h4 className="text-lg font-semibold text-white mb-2">
                      {isMainDragActive ? "Drop your file here" : "Upload your content"}
                    </h4>
                    <p className="text-slate-400 mb-4">
                      Drag and drop your audio, image, or video file here, or click to browse
                    </p>
                    <p className="text-sm text-slate-500">
                      Supports: MP3, WAV, FLAC, JPG, PNG, GIF, MP4, MOV (Max 100MB)
                    </p>
                  </motion.div>
                )}
              </AnimatePresence>
      ...
      {/* Artwork Upload */}
      <div>
        <h3 className="text-lg font-semibold mb-3 text-white">Cover Artwork (Optional)</h3>
        <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
          <Card
            className={`
            border-2 border-dashed transition-all duration-300 cursor-pointer
              ...
              <AnimatePresence mode="wait">
                {uploadedFiles.artwork ? (
                  ...
                  >
                    ...
                    <h4 className="text-lg font-semibold text-white mb-2">{uploadedFiles.artwork.name}</h4>
                    <p className="text-slate-400 mb-4">{(uploadedFiles.artwork.size / 1024 / 1024).toFixed(2)} MB</p>
                    <div className="flex items-center justify-center gap-2">
                      <CheckCircle className="w-5 h-5 text-emerald-400" />
                      <span className="text-emerald-400 font-medium">Uploaded successfully</span>
                    </div>
                  ...
                  >
                    ...
                    <h4 className="text-base font-semibold text-white mb-2">
                      {isArtworkDragActive ? "Drop artwork here" : "Add cover artwork"}
                    </h4>
                    <p className="text-slate-400 text-sm">Upload an image to use as your NFT's cover</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </CardContent>
