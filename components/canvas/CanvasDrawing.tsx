...
export default function CanvasDrawing({ onImageGenerated }: CanvasDrawingProps) {
  ...
  const [selectionToolState, setSelectionToolState] = useState<SelectionToolState>({
    startX: 0,
    startY: 0,
    endX: 0,
    endY: 0,
  ...
  useHotkeys("r", () => setCurrentTool("rectangle"), { enableOnFormTags: false })
  ...
  useHotkeys("i", () => setCurrentTool("eyedropper"), { enableOnFormTags: false })
  useHotkeys("s", () => setCurrentTool("selection"), { enableOnFormTags: false })
  ...
  const calculateConstrainedLine = useCallback((startX: number, startY: number, endX: number, endY: number) => {
  ...
  const calculateConstrainedRect = useCallback((startX: number, startY: number, endX: number, endY: number) => {
    const dx = endX - startX
    const dy = endY - startY
  ...
  const startDrawing = useCallback(
    ...
    [ctx, tempCtx, currentTool, currentColor, isFilled, polygonToolState, fontSize, fontFamily, getPointerPosition],
  )
      ...
      switch (currentTool) {
        ...
        case "line":
          if (lineToolState.isDrawing) {
            ...
            if (isShiftPressed || lineToolState.shiftKey) {
              const constrained = calculateConstrainedLine(lineToolState.startX, lineToolState.startY, x, y)
              endX = constrained.x
              endY = constrained.y
            }
        ...
        case "circle":
          if (shapeToolState.isDrawing) {
            ...
            const centerX = shapeToolState.startX + (endX - shapeToolState.startX) / 2
            const centerY = shapeToolState.startY + (endY - shapeToolState.startY) / 2
        ...
        case "polygon":
          if (polygonToolState.isDrawing && polygonToolState.points.length > 0) {
            // Clear the temp canvas
            clearTempCanvas()

            // Draw the polygon preview
            tempCtx.beginPath()
            tempCtx.moveTo(polygonToolState.points[0].x, polygonToolState.points[0].y)

            // Draw lines between all points
            for (let i = 1; i < polygonToolState.points.length; i++) {
              tempCtx.lineTo(polygonToolState.points[i].x, polygonToolState.points[i].y)
            }
      ...
      switch (currentTool) {
        ...
        case "line":
          if (lineToolState.isDrawing) {
            ...
            if (lineToolState.shiftKey) {
              const constrained = calculateConstrainedLine(lineToolState.startX, lineToolState.startY, endX, endY)
              endX = constrained.x
              endY = constrained.y
            }
        ...
        case "rectangle":
          if (shapeToolState.isDrawing) {
            ...
            if (shapeToolState.shiftKey) {
              const constrained = calculateConstrainedRect(shapeToolState.startX, shapeToolState.startY, endX, endY)
              endX = constrained.x
              endY = constrained.y
            }
            ...
            ctx.beginPath()
            ctx.rect(shapeToolState.startX, shapeToolState.startY, width, height)
        ...
        case "selection":
          ...
          if (selState.isSelecting) {
            ...
            ctx.putImageData(selState.selectedImageData, selState.startX + offsetX, selState.startY + offsetY)
  ...
  const handleDoubleClick = useCallback(() => {
    if (!ctx || !canvasRef.current || currentTool !== "polygon" || !polygonToolState.isDrawing) return
  ...
  }, [ctx, currentTool, polygonToolState, isFilled, clearTempCanvas, saveCanvasState])

  // Function to handle text input submission
  const handleTextSubmit = useCallback(() => {
    if (!ctx || !canvasRef.current || !textToolState.isPlacing || !textInput) return
    ...
    saveCanvasState()
  }, [ctx, textToolState, textInput, currentColor, fontSize, fontFamily, saveCanvasState])
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Shift") {
        ...
        } else if ((currentTool === "rectangle" || currentTool === "circle") && shapeToolState.isDrawing) {
        ...
        if (currentTool === "polygon" && polygonToolState.isDrawing) {
          ...
          setShowTextInput(false)
        } else if (currentTool === "selection" && selectionToolState.selectedImageData) {
          setSelectionToolState({
            startX: 0,
            startY: 0,
            endX: 0,
  ...
  const handleSave = useCallback(() => {
    ...
    if (highResCtx) {
      ...
      highResCanvas.toBlob(
        (blob) => {
          if (blob) {
            const file = new File([blob], "nft-artwork.png", { type: "image/png" })
  ...
  const handleBackgroundChange = useCallback((background: string) => {
    ...
    switch (background) {
      ...
      case "digital-noise":
        ...
        break
      case "neon-gradient":
        canvas.style.backgroundImage = "linear-gradient(135deg, #00f5ff, #9d00ff)"
        break
      case "cyber-gradient":
        canvas.style.backgroundImage = "linear-gradient(135deg, #ff00ff, #00f5ff)"
        break
      case "green-gradient":
        canvas.style.backgroundImage = "linear-gradient(135deg, #39ff14, #00f5ff)"
        break
      case "parchment":
      ...
      case "sketch":
        ...
        break
      case "holographic":
        canvas.style.backgroundImage = "linear-gradient(135deg, #ff00ff, #00f5ff, #39ff14)"
        break
      case "matrix":
      ...
      case "dots":
        canvas.style.backgroundColor = "#ffffff"
        canvas.style.backgroundImage = "radial-gradient(circle, #000000 1px, transparent 1px)"
        canvas.style.backgroundSize = "20px 20px"
        break
      default:
    ...
    <div className="flex flex-col h-full">
      <div className="p-4 flex-grow flex flex-col">
        ...
        <CanvasToolbar
          ...
          canUndoRedo={{ canUndo: history.length > 1, canRedo: redoStack.length > 0 }}
      ...
      {showConfirmClear && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/70 z-50">
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-6 max-w-sm w-full shadow-xl">
            <h3 className="text-lg font-medium text-white mb-4">Clear Canvas?</h3>
